...
function CostPanel(props) {
  ...
  function applyRegion(r){
    ...
    setOvhd(ovhd);
    if(props.onParamsChange) props.onParamsChange({region:r, ovhdPct:ovhd, weldType:weldType});
  }
    ...
    <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:16}}>
      <div style={{overflowY:"auto",maxHeight:680,paddingRight:8}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          <div><Lbl>Welding type</Lbl><Sel value={weldType} onChange={function(v){setWeld(v);if(props.onParamsChange)props.onParamsChange({region:region,ovhdPct:ovhdPct,weldType:v});}} opts={[{v:"thermit",l:"Aluminothermic"},{v:"flash",l:"Flash butt"}]}/></div>
          ...
          <div><Lbl>Overhead (%)</Lbl><Inp value={ovhdPct} onChange={function(v){setOvhd(v);if(props.onParamsChange)props.onParamsChange({region:region,ovhdPct:v,weldType:weldType});}} min={5} max={40}/></div>
        </div>
...
export default function App() {
  ...
  var liveReplRate = useMemo(function(){
    ...
    return c.total;
  },[replRegion, replOvhdPct, replWeldType]);
              ...
              {asr&&(
                <div>
                  ...
                  <div style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:18,border:"1px solid rgba(125,211,200,0.1)",marginBottom:14}}>
                    ...
                    {ctab==="cost"&&<CostPanel simResult={result} horizon={horizon} initRegion={replRegion} initOvhdPct={replOvhdPct} initWeldType={replWeldType} onParamsChange={function(p){if(p.region!==undefined)setReplRegion(p.region);if(p.ovhdPct!==undefined)setReplOvhd(p.ovhdPct);if(p.weldType!==undefined)setReplWeld(p.weldType);}}/>}
