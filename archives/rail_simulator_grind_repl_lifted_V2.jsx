...
function CostPanel(props) {
  ...
  function notifyReplParams(overrides) {
    ...
    props.onParamsChange({region:region, ovhdPct:ovhdPct, weldType:weldType, customP:cur});
  }
  ...
  function applyRegion(r){
    ...
    setOvhd(ovhd);
    if(props.onParamsChange) props.onParamsChange({region:r, ovhdPct:ovhd, weldType:weldType, customP:null});
  }
    ...
    <div style={{display:"grid",gridTemplateColumns:"300px 1fr",gap:16}}>
      <div style={{overflowY:"auto",maxHeight:680,paddingRight:8}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          <div><Lbl>Welding type</Lbl><Sel value={weldType} onChange={function(v){setWeld(v);if(props.onParamsChange)props.onParamsChange({region:region,ovhdPct:ovhdPct,weldType:v,customP:p});}} opts={[{v:"thermit",l:"Aluminothermic"},{v:"flash",l:"Flash butt"}]}/></div>
          ...
          <div><Lbl>Overhead (%)</Lbl><Inp value={ovhdPct} onChange={function(v){setOvhd(v);if(props.onParamsChange)props.onParamsChange({region:region,ovhdPct:v,weldType:weldType,customP:p});}} min={5} max={40}/></div>
        </div>
...
function ComparePanel(props) {
  ...
  var segData = hasComparison ? prevResult.results.map(function(pr, i) {
    ...
    var segReplRate = getReplRate(segGrade);
    var pGrindOp   = lenMl * pPasses * gcp.perMl;
    var cGrindOp   = lenMl * cPasses * gcp.perMl;
    var pMobil = gcp.mobilCostPerInt > 0
      ? (gcp.mobilPerInt ? gcp.mobilCostPerInt * pPasses : gcp.mobilCostPerInt)
      : 0;
    var cMobil = gcp.mobilCostPerInt > 0
      ? (gcp.mobilPerInt ? gcp.mobilCostPerInt * cPasses : gcp.mobilCostPerInt)
      : 0;
        ...
        <div>
          ...
          <div style={{marginBottom:16,borderRadius:10,overflow:"hidden",border:"1px solid "+(totalSaving>0||fhTotalSaving>0?"rgba(125,211,200,0.25)":"rgba(248,113,113,0.25)")}}>
            ...
            <div style={{padding:"10px 18px",background:totalSaving>0||fhTotalSaving>0?"rgba(125,211,200,0.08)":"rgba(248,113,113,0.08)"}}>
              ...
              <div style={{fontSize:11,color:cl.dim,marginTop:3}}>
                Grinding: {gcp.perMl.toFixed(0)} EUR/ml/pass op.{gcp.mobilCostPerInt>0?" + "+Math.round(gcp.mobilCostPerInt)+" EUR mobil/"+(gcp.mobilPerInt?"pass":"horizon"):""}
                {" | "}Replacement: {replRate.toFixed(0)} EUR/ml
              </div>
            </div>
            {/* Two columns */}
          ...
          <div style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:18,border:"1px solid rgba(125,211,200,0.1)",marginBottom:16}}>
            ...
            {chartTab==="cost" && (
              <div>
                <div style={{fontSize:12,color:cl.dim,marginBottom:14}}>Lifecycle cost breakdown - grinding: {gcp.perMl.toFixed(0)} EUR/ml/pass{gcp.mobilCostPerInt>0?" + "+Math.round(gcp.mobilCostPerInt)+" EUR mobil/"+(gcp.mobilPerInt?"pass":"horizon"):""} | replacement: {replRate.toFixed(0)} EUR/ml</div>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                    <thead>
                      <tr style={{background:"rgba(255,255,255,0.03)"}}>
                ...
                <div style={{marginTop:10,fontSize:11,color:"#4a6a74"}}>
                  Rates from your Grinding Cost tab configuration: {gcp.perMl.toFixed(0)} EUR/ml/pass operation{gcp.mobilCostPerInt>0?", "+Math.round(gcp.mobilCostPerInt)+" EUR mobilisation "+(gcp.mobilPerInt?"per intervention":"once per horizon"):""}. Replacement: {replRate.toFixed(0)} EUR/ml from Replacement Cost tab.
                </div>
              </div>
            )}
          </div>
...
export default function App() {
  ...
  var liveReplRate = useMemo(function(){
    ...
    var c = calcCostPerMl(p, "R260", replWeldType, 6, "EUR", replOvhdPct, false, 25);
    return c.total;
  },[replRegion, replOvhdPct, replWeldType, replCustomP]);

  // Grade-specific replacement rate calculator for ComparePanel
  function calcReplRateForGrade(grade) {
    var reg = REGIONS[replRegion] || REGIONS.WEU;
    var p = replCustomP || {lbr:reg.lbr, mat:reg.mat, eqp:reg.eqp, weld:reg.weld, prod:reg.prod, team:reg.team};
    var c = calcCostPerMl(p, grade||"R260", replWeldType, 6, "EUR", replOvhdPct, false, 25);
    return c.total;
  }
              ...
              {asr&&(
                <div>
                  ...
                  <div style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:18,border:"1px solid rgba(125,211,200,0.1)",marginBottom:14}}>
                    ...
                    {ctab==="cost"&&<CostPanel simResult={result} horizon={horizon} initRegion={replRegion} initOvhdPct={replOvhdPct} initWeldType={replWeldType} onParamsChange={function(p){if(p.region!==undefined)setReplRegion(p.region);if(p.ovhdPct!==undefined)setReplOvhd(p.ovhdPct);if(p.weldType!==undefined)setReplWeld(p.weldType);if(p.customP!==undefined)setReplCustomP(p.customP);}}/>}
